...
GCOV_PREFIX_STRIP=1 GCOV_PREFIX=regular ./build/restserver > /dev/null &
RESTSERVER_PID=$!

GCOV_PREFIX_STRIP=1 GCOV_PREFIX=secure ./build/restserver -c ./tests-rest/secure.cfg > /dev/null &
...
gcov-tool merge -o ./ regular secure
